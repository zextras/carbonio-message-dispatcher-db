...
function build-ubuntu-artifact() {
  if [ "$no_docker" = true ]; then
    ...
    docker run \
      --rm --entrypoint "" \
      -v "$(pwd)":/tmp/messaging-db \
      registry.dev.zextras.com/jenkins/pacur/ubuntu-20.04:v1 /bin/bash -c 'pacur build ubuntu /tmp/messaging-db'
  fi
}
...
function build-rocky-8-artifact() {
  if [ "$no_docker" = true ]; then
    ...
    docker run \
      --rm --entrypoint "" \
      -v "$(pwd)":/tmp/messaging-db \
      registry.dev.zextras.com/jenkins/pacur/rocky-8:v1 /bin/bash -c 'pacur build rocky-8 /tmp/messaging-db'
  fi
}
